...
class UpdateProduitCondSortieFieldsListener implements EventSubscriberInterface {
    ...
    {
        $data = $event->getData();
        $produitCond = isset($data['produitCond']) ? $this->em->getRepository(ProduitConditionnement::class)->find($data['produitCond']) : null;
    ...
    )
    {
        ...
        if($produitConditionnement !== null)
        {
            ...
            if($data !== null)
            {
                $magasin = $data->getSortieMag()->getMagasin();
                ...
                $fieldOptions['choice_attr'] = function (ProduitConditionnement $produitConditionnement) use ($prodCondMag, $data) {
                    return [
                        ...
                        'optionValue'       => $prodCondMag->getMagasin()->getId().'-'.$produitConditionnement->getId(),
                    ];
                };
            }
        }
