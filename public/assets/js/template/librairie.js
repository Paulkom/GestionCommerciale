...
function initializeContainer (collectionHolder = null){
    ...
    if(parent != null)
    {
        ...
        inventaireChildren(collectionHolder);
        steppers();
        swishStatut(fieldList,validation);
        produit(parent);
...
function inputEvent(e){
    ...
    if(e.target.hasAttribute('name'))
    {
        ...
        calculMontant('sortie', 'prixUnitaire', 'qteCondSortie');
        calculMontant('approvisionnement', 'prixAchat', 'qteCondAppro');
        calculMontant('commande_frs', 'qteComfrs', 'prixUnitaire');
        controlesTransfert(e);
        controlesSortie(e);
        controlesInventaire(e);
        controlesProduit(e);
    }
}
...
function save(form,validator,btn, erreur) {
    ...
    if (validator) {
        ...
        if (status == 'Valid' && erreur == false) {
            ...
            if($(form).attr('name') === 'facture_proforma'){
                // console.log("Prêt pour le grand saut", $(form).attr('action'));
            }
            $.ajax({
                ...
                if($cont[4]== 'msg_error'){
                    ...
                    if(newform !== '' || newform !== 'undefiny'){
                        ...
                        if($(form).attr('name').includes('commande_client')){
                            $(bloc).html($cont[3]);
                            gestionVente.init();
                            console.log('Bloc de retour',$cont[3]);
                            if($cont[2] == "OUI"){
                                let data = ($cont[1]).toString();
                                let formatP = ($cont[4]).toString();
                                soumission(url,data, null, null, formatP);
                            }
                        }
                        ...
                        if($(form).attr('name').includes('depot')){
                            if($cont[2] == "OUI"){
                                let data = ($cont[1]).toString();
                                soumission(url,data);
                            }
                        ...
                        if($(form).attr('name') === 'compte'){
                            newform = $cont[3];
                            GESTIONNAIRECOMPTE.init();
                            // console.log(newform,"fefefef",  $cont[3]);
                        }
                    }
                }
...
function onEdit(elt, data) {
    ...
    if($(form).attr('name')==='client'){
        swishStatut(fieldList,validation);
    }
    if($(form).attr('name') == 'produit'){
        checkEstService();
        checkEstCarreau();
        //reactiver le mode carreau s'il
        // let element = $(form).find('input[name*="estModeCarreau"]');
        // if(element.is(':checked')){
        //     element.closest('.box').addClass('bg-light-primary');
